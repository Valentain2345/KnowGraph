...
\section{Motivación y proyectos similares}
...
\subsection{Proyectos y herramientas relevantes}

\begin{itemize}
    \item \textit{Protégé}: es un editor de ontologías de código abierto y un sistema de gestión de conocimiento desarrollado por la Universidad de Stanford. Protégé\cite{protege} permite definir modelos conceptuales formales (ontologías) y sus relaciones, facilitando la construcción de esquemas semánticos exportables a formatos como OWL y RDF, que pueden ser usados como base para grafos de conocimiento. 

   \item \textit{MindBugs Discovery}: es una plataforma que integra técnicas de inteligencia artificial, minería de datos y análisis semántico para ayudar a investigadores, periodistas y analistas a explorar y comprender narrativas complejas —particularmente aquellas relacionadas con la desinformación\footnote{En este caso, se entiende por desinformación la dispersión dentro de la población de información falsa o engañosa para asegurar una ganancia económica o política, y que puede causar un daño público. Cabe destacar que en el sitio oficial de la herramienta no se define explícitamente el término, pero por el contexto del proyecto esta definición es adecuada. \cite{mindbugs2026discovery}.}— mediante visualizaciones interactivas y grafos que muestran conexiones entre entidades, eventos y datos verificados. La herramienta construye un conocimiento estructurado a partir de fuentes verificadas y permite explorar relaciones semánticas entre conceptos dentro de un grafo interactivo.\cite{mindbugs2026discovery}

    \item \textit{Stardog}: es una plataforma de base de datos semántica empresarial que permite almacenar y consultar grafos de conocimiento basados en RDF con capacidades avanzadas de razonamiento e integración de datos. Stardog\cite{stardog} incluye un motor que soporta consultas SPARQL complejas y razonamiento lógico sobre los datos, habilitando consultas federadas y análisis semántico profundo. 
    
    \item \textit{Wikidata Query Creator}: vinculado a \textit{Wikidata}\cite{wikidata}, una base de conocimiento abierta mantenida por la Fundación Wikimedia, permite realizar consultas complejas sobre una enorme red de entidades y relaciones utilizando SPARQL, facilitando la extracción de información interconectada y su análisis semántico.

    \item \textit{DBpedia Query Creator}: asociado a \textit{DBpedia}\cite{dbpedia}, un proyecto que extrae datos estructurados de Wikipedia y los convierte en un grafo de conocimiento accesible mediante consultas SPARQL. Esto permite explorar las relaciones semánticas entre miles de entidades extraídas de la enciclopedia, habilitando aplicaciones de consulta y visualización de datos.

    \item  \textit{Knowledge Graph de Google}: Como referente fundamental en la aplicación masiva de estas tecnologías a nivel industrial, es imperativo analizar el Knowledge Graph de Google\cite{google2012knowledge}. Este caso de estudio demuestra la viabilidad y el impacto de los grafos de conocimiento a gran escala, sirviendo como motivación para el desarrollo de herramientas de exploración visual como KnowGraph.

\end{itemize}

Estas herramientas y plataformas representan enfoques diversos dentro del ecosistema de conocimiento estructurado y grafos de conocimiento: desde editores y modeladores de ontologías (\textit{Protégé}), hasta plataformas que integran análisis semántico y visualizaciones avanzadas (\textit{MindBugs Discovery}) o potentes motores de almacenamiento y consulta de conocimiento (\textit{Stardog}, \textit{Wikidata} y \textit{DBpedia}). Todos ellos comparten el objetivo de organizar, interrogar y explotar datos interrelacionados, lo que los hace útiles para comparar, contrastar y fundamentar el desarrollo de KnowGraph.

\subsubsection*{Knowledge Graph de Google}
...
El término \textit{Linked Data} (Datos Enlazados) hace referencia a un conjunto de buenas prácticas formalizadas para publicar y conectar datos estructurados en la Web. Propuesto inicialmente por Tim Berners-Lee en su artículo fundacional \cite{berners2006linked}, este enfoque busca transformar la Web tradicional —concebida originalmente como un espacio de documentos legibles por humanos— en una Web de Datos (\textit{Web of Data}) interoperables y comprensibles directamente por agentes computacionales.
...
\begin{enumerate}
    \item \textbf{Uso de URIs como nombres:} Utilizar Identificadores de Recursos Uniformes (URIs) de manera unívoca para identificar cualquier tipo de entidad u objeto del mundo real (personas, lugares, conceptos abstractos).
    \item \textbf{Adopción de URIs bajo el protocolo HTTP:} Garantizar que dichas URIs empleen el protocolo HTTP, permitiendo que usuarios y sistemas puedan resolver y consultar la identificación de dichos nombres de manera distribuida.
    \item \textbf{Provisión de información útil mediante estándares:} Al resolver una URI, el sistema debe proporcionar información útil y contextualizada sobre el recurso utilizando estándares semánticos abiertos, específicamente el modelo RDF (\textit{Resource Description Framework}) y el lenguaje de consulta SPARQL.
    \item \textbf{Inclusión de enlaces salientes:} Incorporar enlaces hacia otras URIs externas y relacionadas dentro de los datos publicados, permitiendo el descubrimiento orgánico de nuevo conocimiento a lo largo de la Web.
\end{enumerate}

La implementación conjunta de estos principios permite romper los silos de información y estructurar grafos de datos globales. Como se ilustra en la Figura \ref{fig:linkeddata}, el paradigma facilita la transición desde bases de datos aisladas hacia un tejido de conocimiento interconectado de carácter público y descentralizado.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\linewidth]{Diagramas/LinkedData.png}
    \caption[Representación de Linked Data]{Representación de Linked Data\footnotemark}
    \label{fig:linkeddata}
\end{figure}
\footnotetext{\textbf{Fuente:} Comune di Firenze. \textit{Linked Open Data}. Disponible en: \href{https://opendata.comune.fi.it/content/linked-open-data}{opendata.comune.fi.it/content/linked-open-data}}

Adicionalmente, cuando los principios de Linked Data se aplican sobre datos con licencias de acceso abierto, se acuña el término \textit{Linked Open Data} (LOD). Para evaluar el nivel de madurez en su publicación, se utiliza un sistema de clasificación de cinco estrellas:

\begin{itemize}
    \item \textbf{1 Estrella ($\star$):} Datos publicados en la Web bajo una licencia abierta, independientemente del formato (ej. un PDF escaneado).
    \item \textbf{2 Estrellas ($\star\star$):} Datos estructurados en un formato legible por máquina, pero propietario (ej. una hoja de cálculo en formato Excel de Microsoft).
    \item \textbf{3 Estrellas ($\star\star\star$):} Datos en formatos estructurados y no propietarios basados en estándares abiertos (ej. archivos en formato CSV).
    \item \textbf{4 Estrellas ($\star\star\star\star$):} Datos que emplean estándares del consorcio W3C como URIs y RDF para identificar y describir recursos.
    \item \textbf{5 Estrellas ($\star\star\star\star\star$):} Datos que cumplen con todos los criterios anteriores e incorporan activamente enlaces salientes hacia datos de otros proveedores, maximizando el valor del contexto semántico.
\end{itemize}
...
Los \textit{grafos de conocimiento} son estructuras de representación del conocimiento que combinan la flexibilidad de los modelos de grafos con la semántica explícita de las relaciones y entidades, permitiendo modelar, integrar y explotar grandes volúmenes de información heterogénea. Han experimentado un crecimiento notable tanto en la investigación académica como en aplicaciones industriales, debido a su capacidad para representar datos interconectados a gran escala y facilitar operaciones de razonamiento, integración y consulta sobre estos datos.

Esta infraestructura de datos abiertos y estructurados bajo el estándar de las cinco estrellas proporciona el entorno técnico ideal para la evolución de los modelos de información. A partir de estas bases de enlazado y conectividad distribuida, cobra relevancia el concepto de grafo de conocimiento, estructura que se detalla en el próximo apartado como el núcleo formal para la representación del conocimiento del proyecto.

\subsection{Definición y caracterización}

Aunque no existe una definición única universalmente aceptada, los grafos de conocimiento se pueden entender, en términos generales, como estructuras de datos que representan \textit{entidades} explícitas y las \textit{relaciones} entre ellas mediante nodos y aristas en un grafo semántico. Además, incorporan mecanismos formales de vocabulario (esquemas o ontologías) y, en muchos casos, capacidades de razonamiento automático para derivar conocimiento implícito a partir de datos explícitos.

En la literatura técnica revisada por Hogan et al.\cite{hogan2020knowledgegraphs}, se identifican diversas perspectivas sobre qué constituye un grafo de conocimiento. Algunas definiciones se centran en componentes técnicos específicos, como una combinación de:
\begin{itemize}
...
\begin{itemize}
    \item \textit{Entidades}: Representan objetos del dominio de interés (personas, lugares, conceptos, eventos, etc.).
    \item \textit{Relaciones}: Aristas etiquetadas que describen interacciones o conexiones significativas entre entidades.
    \item \textit{Atributos y literales}: Valores asociados a entidades o relaciones que describen propiedades concretas.
    \item \textit{Esquemas u ontologías}: Vocabularios formales que definen clases, propiedades y restricciones, proporcionando una base semántica para la interpretación de los datos.
\end{itemize}
...
\begin{itemize}
    \item \textit{Extracción de conocimiento}: Identificación de entidades y relaciones relevantes a partir de fuentes heterogéneas de datos.
    \item \textit{Enriquecimiento y alineamiento}: Integración de datos nuevos y alineamiento con vocabularios o esquemas existentes.
    \item \textit{Evaluación de la calidad}: Verificación de consistencia, completitud y corrección de las declaraciones en el grafo.
    \item \textit{Publicación y mantenimiento}: Difusión del grafo de conocimiento a través de servicios, APIs o endpoints de consulta y su actualización continua conforme evolucionan los datos subyacentes.
\end{itemize}
...
El \textit{Resource Description Framework (RDF)} es un estándar recomendado por el \textit{World Wide Web Consortium (W3C)} para la \textit{representación y el intercambio de información en la Web Semántica}. Fue desarrollado con el propósito de proporcionar un \textit{modelo de datos flexible, extensible y orientado a grafos} que permita describir recursos y sus relaciones de forma que puedan ser interpretados y procesados por máquinas de manera interoperable en entornos distribuidos. El estándar RDF ha evolucionado a lo largo del tiempo, con versiones aprobadas como RDF 1.0 y RDF 1.1, consolidándose como una de las piedras angulares de la Web Semántica y la infraestructura de datos enlazados (\textit{Linked Data}).

\subsection{Modelo de datos RDF}

La base del modelo RDF es un \textit{grafo dirigido etiquetado} compuesto por estructuras denominadas \textit{triplas} o \textit{triples} (del inglés \textit{triples}): cada tripla está formada por un \textit{sujeto}, un \textit{predicado} y un \textit{objeto}, que conjuntamente representan una \textit{afirmación semántica} sobre algún dominio de conocimiento. En esta representación:

\begin{itemize}
    \item El \textit{sujeto} identifica el recurso que se está describiendo.
    \item El \textit{predicado} (o propiedad) indica la \textit{relación} o característica del sujeto.
    \item El \textit{objeto} corresponde al valor de esa característica o al recurso con el que se relaciona el sujeto.
\end{itemize}

Cada uno de estos componentes puede ser identificado mediante un \textit{Identificador de Recursos Uniforme (URI)} (por ejemplo, \texttt{http://xmlns.com/foaf/0.1/Person}), lo que permite referenciar inequívocamente recursos distribuidos a lo largo de la Web. El objeto también puede ser un \textit{valor literal} (por ejemplo, una cadena de texto, un número o una fecha).

Conceptualmente, un conjunto de triples puede representarse como un \textit{grafo RDF} en el que los nodos representan recursos o literales y las aristas etiquetadas representan las relaciones semánticas entre ellos. Esta estructura de grafo posibilita la representación de información compleja y altamente interconectada de forma natural y escalable.

\subsection{Formalización de RDF como Grafos}

El modelo de datos RDF se define formalmente en términos de grafos abstractos, en los cuales cada tripla constituye una \textit{arista dirigida y etiquetada} que conecta dos nodos. El conjunto de todas las triplas constituye un \textit{grafo RDF}, que puede visualizarse como una red de relaciones entre entidades. Esta abstracción matemática permite aplicar técnicas de teoría de grafos y lógica para razonamiento, integración y consulta de información.

Un \textit{grafo RDF} puede describir hechos simples, como por ejemplo:
...
Una de las ventajas fundamentales de RDF es su \textit{capacidad para representar conocimiento sin imponer un esquema rígido} ni una ontología prescrita. Los grafos RDF pueden crecer de forma incremental y pueden integrar datos de múltiples fuentes heterogéneas, lo que facilita la \textit{interoperabilidad}, la \textit{extensibilidad} y la \textit{reutilización} de información. Asimismo, dado que los nombres de recursos y propiedades están basados en URIs, es posible establecer enlaces entre datos distribuidos, característica esencial del paradigma \textit{Linked Data}.

El modelo RDF también permite describir los datos en diferentes \textit{formatos de serialización}, como RDF/XML, Turtle, N-Triples, JSON-LD, entre otros. Estos formatos permiten tanto la representación textual como la transmisión de datos RDF de manera legible por máquinas y estandarizada.

\subsection{Conexión con grafos de conocimiento}

La representación de información como grafos RDF es un \textit{elemento central para la construcción de grafos de conocimiento}, ya que permite modelar de forma explícita las entidades y sus relaciones semánticas. Un \textit{grafo de conocimiento} no es más que un conjunto de recursos interconectados mediante relaciones, formalizado como un \textit{grafo RDF enriquecido con vocabularios y ontologías} que aportan semántica adicional. Esta estructura facilita no solo la \textit{recuperación eficiente de información} mediante lenguajes de consulta como SPARQL, sino también la \textit{inferencias de nuevo conocimiento}, especialmente cuando se combinan RDF con esquemas de ontologías (por ejemplo, RDFS y OWL) y mecanismos de razonamiento semántico.


En este sentido, RDF actúa como una \textit{capa base de representación} que proporciona la semántica estructural necesaria para que sistemas automatizados puedan integrar, enlazar y explotar datos de manera coherente en la Web Semántica. Esta capacidad de formalizar el significado y las relaciones entre recursos convierte a RDF en un pilar fundamental para la Web de Datos, facilitando aplicaciones avanzadas como motores de búsqueda semántica, asistentes inteligentes, análisis de redes de información y sistemas de integración de datos a gran escala.
...
De acuerdo con DuCharme \cite[p. 115]{ducharme2013learning}, crear nuevos datos a partir de datos existentes es uno de los aspectos más interesantes de la tecnología SPARQL y RDF. \cite[p. ] {ducharme2013learning} La capacidad de escribir y ejecutar una sola consulta que recupera datos de múltiples conjuntos de datos... ya sabes todo lo que necesitas saber: solo crea una subconsulta para cada uno.
...
\begin{itemize}
    \item \textit{Lenguaje declarativo orientado a grafos}:  
    ...
    \item \textit{Sintaxis inspirada en SQL}:  
    ...
    \item \textit{Patrones de triplas y variables}:  
   ...
   \item \textit{Formas de consulta [H]} (donde [H] se reemplaza por una de las directivas listadas a continuación):
    \begin{itemize}
    ...
    \item \textit{Extensiones de SPARQL 1.1}:  
...
A modo integrador, un ejemplo de consulta básica extraída de la literatura \cite[p. 3]{ducharme2013learning} para obtener nombres de personas se estructura de la siguiente manera:
...
\begin{itemize}
    \item \textit{Interacción con almacenes de triplas (Triplestores)}: Provee el canal estandarizado para ejecutar operaciones de lectura y escritura sobre motores de persistencia semántica como Apache Jena o Stardog.
    \item \textit{Integración de datos heterogéneos}: Facilita la unificación de repositorios distribuidos mediante federación de consultas, resolviendo conflictos de esquemas a través de la Web Semántica.
    \item \textit{Explotación de bases de conocimiento públicas}: Permite interrogar de forma directa grandes iniciativas globales de datos enlazados, tales como Wikidata y DBpedia.
    \item \textit{Inferencia y descubrimiento de conocimiento}: Actúa en coprocesamiento con motores de razonamiento lógico para interrogar axiomas implícitos deducidos a partir de las ontologías cargadas.
\end{itemize}

La capacidad de SPARQL para interrogar almacenes de triplas y federar consultas demuestra el potencial práctico del lenguaje en entornos reales. Sin embargo, para materializar estas operaciones dentro de la arquitectura del software y gestionar eficazmente el almacenamiento y manipulación de los datos semánticos, es indispensable incorporar un framework especializado en el backend. Por este motivo, el siguiente apartado se dedica al estudio de \textit{Apache Jena}, la herramienta que actúa como puente de desarrollo en el proyecto KnowGraph.

\section{Apache Jena}

Apache Jena\cite{jena} es un \textit{marco de trabajo de código abierto para Java} diseñado para construir aplicaciones orientadas a la Web Semántica y Linked Data. Proporciona \textit{APIs completas para la manipulación, almacenamiento, consulta y publicación de datos RDF}, así como soporte para ontologías, razonamiento y servicios SPARQL. Su diseño modular permite integrar múltiples componentes especializados que, en conjunto, constituyen una plataforma potente para desarrollar soluciones basadas en grafos de conocimiento y tecnologías semánticas.
...
\begin{itemize}
    \item \textit{RDF API}: Es la API central de Jena para crear, manipular y serializar grafos RDF. Proporciona abstracciones de más alto nivel como \texttt{Resource}, \texttt{Literal}, \texttt{Statement} y \texttt{Model}, facilitando la construcción y consulta de triplas RDF en memoria o en almacenamiento persistente.
    \item \textit{SPARQL (ARQ)}: Jena integra un motor de consulta y actualización compatible con SPARQL 1.1 llamado ARQ, lo que permite ejecutar consultas SPARQL sobre modelos RDF locales o remotos y sobre datasets persistentes.
    \item \textit{TDB/TDB2}: Son componentes de almacenamiento de triplas RDF de alto rendimiento diseñados para persistir datos directamente en disco. Jena incluye TDB2 como su versión más reciente y optimizada de almacenamiento persistente, soportando transacciones y manejo eficiente de grandes volúmenes de datos RDF.

    \item \textit{Fuseki}: Es un \textit{servidor SPARQL} que expone datasets RDF mediante \textit{endpoints HTTP}, permitiendo consultas SPARQL y actualizaciones remotas conforme a los estándares SPARQL 1.1 y SPARQL Graph Store Protocol. El servidor puede ejecutarse como servicio independiente, aplicación web empaquetada (WAR) o integrado en otras aplicaciones.

    \item \textit{Ontología e inferencia}: Jena provee APIs específicas para trabajar con \textit{ontologías OWL y RDFS}, así como un motor de inferencia que permite derivar conocimiento implícito a partir de hechos explícitos, mediante reglas de razonamiento predefinidas o personalizadas.

    \item \textit{Otros módulos auxiliares}: Jena incluye funcionalidades adicionales como procesadores SHACL y ShEx para validación de esquemas, soporte GeoSPARQL para datos geoespaciales, búsqueda de texto indexado (Lucene/Solr), APIs para conexiones unificadas a datasets (\texttt{RDFConnection}), herramientas de línea de comandos y generadores de código (QueryBuilder).
\end{itemize}

\subsection{Arquitectura y modelos de datos}

La esencia de Jena radica en representar datos RDF como \textit{grafos dirigidos de triplas} compuestos por sujetos, predicados y objetos. Su API de RDF abstrae estas estructuras de forma que aplicaciones Java puedan:
...
El subsistema de inferencia permite \textit{enriquecer datos} utilizando sistemas de reglas, ya sean basados en RDFS/OWL o personalizados, de modo que las conclusiones derivadas se reflejen en los modelos de forma transparente para el desarrollador.
...
Fuseki se posiciona como una pieza clave para \textit{servir grafos de conocimiento en entornos distribuidos}, habilitando a los consumidores de datos (otros sistemas o aplicaciones) a acceder y manipular datos RDF de forma remota sin acoplamiento directo a los formatos de almacenamiento subyacentes.
...
\section{Tecnologías de soporte de la programación usadas}
...
Nginx\cite{nginx} es un servidor web de alto rendimiento y código abierto que también funciona como proxy inverso, balanceador de carga, caché de contenido y proxy para protocolos TCP/UDP y correo.Fue creado por Igor Sysoev y se distribuye bajo la licencia BSD de 2 cláusulas. Está diseñado para manejar un gran número de conexiones concurrentes con un consumo mínimo de recursos, lo que lo convierte en una opción popular para sitios web y aplicaciones de alta demanda.
...
\begin{itemize}
\item \textbf{Balanceo de carga}: Distribuye el tráfico entrante entre varios servidores backend utilizando algoritmos como round-robin, least connections o IP hash, evitando la sobrecarga de un único servidor y mejorando la disponibilidad.
\item \textbf{Caché de contenido}: Almacena respuestas de los servidores backend (como páginas estáticas o resultados de consultas) y las sirve directamente a los clientes, reduciendo la carga en los servidores y acelerando los tiempos de respuesta.
\item \textbf{Terminación SSL/TLS}: Gestiona el cifrado y descifrado de las conexiones seguras, liberando a los servidores backend de esta tarea y centralizando la gestión de certificados .
\item \textbf{Seguridad}: Oculta la infraestructura interna al actuar como intermediario, protegiendo los servidores backend de ataques directos y permitiendo implementar políticas de seguridad centralizadas, como listas de denegación o limitación de tasa.

\end{itemize}

Nginx utiliza un modelo de procesos asíncrono y basado en eventos, con un proceso maestro que gestiona múltiples procesos worker, los cuales realizan el procesamiento real de las solicitudes. Su configuración es flexible y modular, organizada en directivas y contextos (como \texttt{http}, \texttt{server}, \texttt{location}) que permiten un control preciso del comportamiento del servidor.
La interacción coordinada entre el servidor web y los servicios de backend requiere una estrategia de despliegue que garantice la portabilidad y la homogeneidad del entorno de ejecución, objetivos que se alcanzan mediante la contenedorización con Docker.
...
\begin{itemize}
    \item \textbf{Aislamiento}: Cada servicio se ejecuta en su propio contenedor, con sus propias dependencias, evitando conflictos entre versiones de bibliotecas o entornos de ejecución.
    \item \textbf{Portabilidad}: Las imágenes de Docker pueden ejecutarse en cualquier máquina que tenga Docker, desde el ordenador de un desarrollador hasta servidores en la nube, facilitando el despliegue y la colaboración.
    \item \textbf{Escalabilidad}: Los servicios pueden escalarse de forma independiente añadiendo más instancias de un contenedor, especialmente útil para los microservicios de visualización que pueden requerir mayor capacidad de procesamiento.
    \item \textbf{Consistencia}: El entorno de desarrollo, prueba y producción es idéntico, eliminando los problemas de “funciona en mi máquina”.
    \item \textbf{Gestión simplificada}: Con Docker Compose se define y orquesta toda la arquitectura de servicios en un único archivo YAML, permitiendo levantar el sistema completo con un solo comando.
\end{itemize}

La adopción de Docker ha sido fundamental para lograr una arquitectura de microservicios desacoplada, fácil de mantener y desplegar. 
La infraestructura de contenedores asegura que los servicios base estén perfectamente orquestados. Sobre esta base sólida de backend y despliegue, la sección posterior aborda las tecnologías seleccionadas para la construcción de la capa de presentación e interfaz de usuario.

\subsection{Vite, Typescript y React para construir un frontend profesional}
...
\textit{Vite}\cite{vite} es una herramienta de construcción (\textit{build tool}) y servidor de desarrollo moderna orientada a proyectos web. Fue creada para proporcionar un flujo de desarrollo rápido y eficiente aprovechando los módulos ES nativos del navegador. Durante el desarrollo, Vite actúa como un servidor que sirve los archivos directamente sin necesidad de realizar un empaquetado completo, permitiendo tiempos de arranque casi instantáneos y recargas en caliente (Hot Module Replacement, HMR). Esto mejora considerablemente la experiencia del desarrollador. Para producción, Vite utiliza herramientas como Rollup para generar paquetes optimizados, con código más pequeño y eficiente.

\subsubsection*{TypeScript}

\textit{TypeScript}\cite{typescript} es un lenguaje de programación de alto nivel que se basa en JavaScript pero añade tipado estático opcional. Fue desarrollado por Microsoft y permite a los desarrolladores definir tipos de datos para variables, parámetros y estructuras más complejas. Este sistema de tipos se verifica en tiempo de compilación, lo que ayuda a detectar errores antes de ejecutar la aplicación y mejora la mantenibilidad y claridad del código. TypeScript se transpila a JavaScript estándar para ser ejecutado por los navegadores o entornos como Node.js.

\subsubsection*{React}

\textit{React}\cite{react} es una biblioteca de JavaScript de código abierto diseñada para construir interfaces de usuario (\textit{user interfaces}). Fue desarrollada por Meta (antes Facebook) y se utiliza ampliamente para crear aplicaciones web interactivas y dinámicas. React se basa en una arquitectura de componentes reutilizables: cada componente representa una parte específica de la interfaz de usuario y puede gestionar su propio estado. Además, React utiliza una representación virtual del DOM (\textit{Virtual DOM}) para actualizar de forma eficiente solo las partes de la interfaz que han cambiado, reduciendo el trabajo innecesario al interactuar con la página.
...
La aplicación de técnicas de reducción de dimensionalidad en Python se realiza comúnmente mediante librerías especializadas como \texttt{scikit-learn} o \texttt{umap-learn}\cite{umaplearn}, que proveen implementaciones de PCA, t-SNE y UMAP. Estas herramientas permiten ajustar los datos originales y transformar las coordenadas al espacio reducido deseado, generando así el conjunto de puntos que será visualizado con D3.js o Three.js.



\section{Comportamiento de la aplicación y manual de usuario}
Una vez desarrollados los conceptos teóricos necesarios, a continuación se describe el funcionamiento de la aplicación \textit{KnowGraph} que va a servir como manual de usuario de la aplicación.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/QueryExecutor.png}
    \caption{Pantalla de ejecucion de consultas}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/QueryResults.png}
    \caption{Pantalla luego de ejecutar una consulta}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\linewidth]{Diagramas/consulta.png}
    \caption{Diagrama secuencia ejecución de una consulta}
\end{figure}
En la parte superior derecha se encuentra el botón de \textit{Help} que al seleccionarlo despliega un menu que propone dos ejemplos al hacer clic en alguno de ellos se cargara un conjunto de datos de ejemplo en el servidor y una consulta de ejemplo en el cliente para que ejecute. En el resto de la sección de funcionamiento de la aplicación se usarán estos conjuntos de datos como ejemplos y sus respectivas consultas asociadas.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/HelpButton.png}
    \caption{Ejemplos de conjuntos de datos}
\end{figure}
...
El desarrollo de consultas SPARQL (SPARQL Protocol and RDF Query Language) tradicionalmente requiere conocimiento especializado en la sintaxis del lenguaje, lo que representa una barrera de entrada para usuarios no técnicos. Para abordar este problema, se ha desarrollado un generador visual de consultas SPARQL que implementa un enfoque \textit{wizard} (asistente) para la construcción de consultas. Este sistema permite a usuarios sin experiencia en SPARQL construir consultas complejas a través de una interfaz gráfica intuitiva, mientras genera código SPARQL válido y optimizado en segundo plano.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/PrefixManager.png}
    \caption{Gestor de prefijos en el generador de consultas}
\end{figure}
...
\begin{enumerate}
    \item \textit{Gestor de Prefijos}: Administra las declaraciones de espacios de nombres (namespaces) utilizados en la consulta.
    \item \textit{Gestor de Conjuntos de Datos}: Configura los grafos RDF fuente (tanto grafos por defecto como nombrados).
    \item \textit{Constructor de Consultas}: Interfaz principal para la definición de patrones de triplas, filtros, funciones y cláusulas adicionales.
\end{enumerate}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/DatasetManager.png}
    \caption{Gestor de conjuntos de datos}
\end{figure}
...
\begin{itemize}
    \item \textit{Nombre del Prefijo}: Identificador corto (ej. \texttt{foaf})
    \item \textit{URI del Espacio de Nombres}: URI completa del vocabulario (ej. \texttt{http://xmlns.com/foaf/0.1/})
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/QueryBuilder.png}
    \caption{Constructor de consultas, interfaz prinxipal}
\end{figure}
\textit{Ejemplo de configuración:}
\begin{verbatim}
...
\begin{itemize}
    \item \textit{Grafos por Defecto}: Fuentes principales de datos (cláusula \texttt{FROM})
    \item \textit{Grafos Nombrados}: Grafos específicos identificados por URI (cláusula \texttt{FROM NAMED})
\end{itemize}
...
\hline
\textit{Componente} & \textit{Descripción} & \textit{Ejemplo} \\
\hline
...
Para construir una tripla simple, el usuario completa:
\begin{itemize}
    \item \textit{Sujeto}: Nombre de variable (sin el prefijo \texttt{?})
    \item \textit{Propiedad}: Predicado con prefijo opcional
    \item \textit{Alias}: Variable para el objeto
\end{itemize}

Ejemplo: \texttt{?person foaf:name ?personName}

\paragraph{Entidades Tipadas:}
Para consultar entidades de un tipo específico, se completa adicionalmente el campo \textit{Concepto}. El sistema genera automáticamente la tripla de tipo RDF: \texttt{?person rdf:type foaf:Person}
...
\begin{itemize}
    \item \textit{DISTINCT}: Eliminar duplicados de los resultados
    \item \textit{LIMIT}: Número máximo de resultados (paginación)
    \item \textit{OFFSET}: Desplazamiento para paginación
    \item \textit{ORDER BY}: Ordenamiento basado en variables visibles
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/EjemploConsultaGenerada.png}
    \caption{Ejemplo de consulta generada con el constructor}
\end{figure}
...
Mientras que la sección anterior abordaba la construcción de consultas SELECT para extraer información de grafos RDF, el \textit{Triple Builder} (Constructor de triplas) se enfoca en la creación de consultas de inserción de datos. Este módulo permite construir consultas \texttt{INSERT DATA} de SPARQL, que son fundamentales para la carga y actualización de datos en repositorios RDF. 
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/TripleBuilder.png}
    \caption{Imagen al entrar al constructor de triplas}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/TripleBuilderSecondPart.png}
    \caption{Constructor de triplas segunda parte}
\end{figure}
...
\begin{enumerate}
    \item \textit{Gestor de Prefijos}: Heredado del sistema anterior, permite definir espacios de nombres para abreviar URIs en las triplas.
    \item \textit{Constructor de triplas Individuales}: Interfaz para definir cada componente de una tripla RDF (sujeto, predicado, objeto).
    \item \textit{Administrador de triplas Construidas}: Panel donde se visualizan, organizan y gestionan todas las triplas antes de generar la consulta.
    \item \textit{Selector de Grafo Destino}: Mecanismo para especificar el grafo RDF donde se insertarán las triplas.
\end{enumerate}
...
\begin{itemize}
    \item \textit{Nombre}: Abreviatura (ej. \texttt{foaf})
    \item \textit{URI}: Identificador completo del espacio de nombres (ej. \texttt{http://xmlns.com/foaf/0.1/})
\end{itemize}
...
\begin{itemize}
    \item \textit{Selector de Prefijo}: Lista desplegable con los prefijos definidos
    \item \textit{Campo de Texto}: Para ingresar el valor local del componente
    \item \textit{Lista de Sugerencias}: Valores predefinidos para acelerar la entrada
\end{itemize}

\textit{Ejemplo de construcción:}
\begin{itemize}
...
\begin{itemize}
    ...
    \item La estructura completa de cada tripla (sujeto, predicado, objeto)
...
\begin{itemize}
    \item \textit{Campo de Texto}: Para ingresar manualmente la URI del grafo
    \item \textit{Lista Desplegable}: Con grafos predefinidos (ej. \texttt{http://example.org/graph/people})
    \item \textit{Botón de Reinicio}: Para limpiar la selección actual
\end{itemize}
...
\begin{enumerate}
    \item Valida que exista al menos una tripla construida
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/ConstructedTriple.png}
    \caption{Tripla hecha con el constructor}
\end{figure}
\subsection{Ejemplos de Uso}
...
\textit{Consulta Generada (fragmento):}
\begin{verbatim}
PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
PREFIX foaf: <http://xmlns.com/foaf/0.1/>
PREFIX ex: <http://example.org/>
...
\begin{itemize}
    ...
    \item Comprobación de que al menos una tripla esté presente para generar la consulta
\end{itemize}
...
\begin{enumerate}
    \item \textit{Compartición de Prefijos}: Los prefijos definidos en este módulo pueden ser utilizados por otros componentes del sistema.
    \item \textit{Flujo Unificado}: La consulta generada se envía automáticamente al ejecutor de SPARQL.
    \item \textit{Consistencia de Interfaz}: Mantiene el mismo diseño visual y patrones de interacción que el generador de consultas SELECT.
    \item \textit{Mensajería Unificada}: Utiliza el mismo sistema de notificaciones para informar sobre el estado de las operaciones.
\end{enumerate}

\section{Cambio de proveedor de servicios de SPARQL y datos}

El botón que despliega el menú \textit{Help} incluye, además de los ejemplos, una sección denominada \textit{SPARQL Provider}. Al posicionar el cursor sobre esta opción se muestran cuatro alternativas de proveedor: \textbf{Know Graph} (seleccionado por defecto), \textbf{Wikidata}, \textbf{DBpedia} y \textbf{Custom}.  

\textbf{Know Graph} constituye el \textit{backend} principal de la aplicación y ofrece la mayor compatibilidad de servicios, incluyendo dos consultas de ejemplo integradas. Al cambiar a \textbf{Wikidata} o \textbf{DBpedia}, el sistema adapta dinámicamente la interfaz: el menú de visualización permanece accesible, mientras que otras opciones (como los ejemplos predefinidos adicionales) se desactivan, indicando visualmente que no están soportadas por esos endpoints. En estos casos, únicamente se encuentra disponible un ejemplo de consulta. La funcionalidad de importación, sin embargo, continúa operativa independientemente del proveedor seleccionado, permitiendo cargar consultas externas.  

La opción \textbf{Custom} permite al usuario agregar un punto SPARQL personalizado. Al seleccionarla se abre un panel de configuración (Figura~\ref{fig:panel-custom}) donde se solicitan la URL del \textit{endpoint}, el modo de envío de la consulta y el formato de respuesta esperado. Una vez introducidos los parámetros, la aplicación intenta establecer la conexión. Para los proveedores personalizados no se ofrecen ejemplos predefinidos, pero todas las funcionalidades de ejecución y visualización se mantienen habilitadas una vez configurado el nuevo origen de datos.  

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes Aplicacion/ProveedoresSparql.png}
    \caption{Opciones de proveedores SPARQL soportados en el menú \textit{Help}.}
    \label{fig:proveedores-sparql}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Imagenes Aplicacion/PanelProveedorSparqlPersonalizado.png}
    \caption{Panel de configuración de un proveedor SPARQL personalizado.}
    \label{fig:panel-custom}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/VisualizationButton.png}
    \caption{Botón de visualización, menú de reducción dimensional seleccionado}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/VisualizationButtonGraph.png}
    \caption{Botón de visualización, menú de reducción generar grafo seleccionado}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/2dGraph.png}
    \caption{Ejemplo de grafo en 2D}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/GraphIn3d.png}
    \caption{Ejemplo de grafo en 3D}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/GraphSelector1.png}
    \caption{Primera paso constructor de grafos}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/GraphSelector2.png}
    \caption{Segundo paso constructor de grafos}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/Visualization2d.png}
    \caption{Ejemplo de umap en 2D}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/Visualization3d.png}
    \caption{Ejemplo de umap en 3D}
\end{figure}
Al seleccionar cualquiera de estos métodos, se abre un panel en el que se pueden elegir las variables que serán utilizadas para realizar la reducción dimensional. Para generar una visualización en dos dimensiones se requiere seleccionar al menos dos variables, mientras que para una visualización en tres dimensiones se deben seleccionar tres o más. La cantidad de dimensiones se define mediante un menú desplegable ubicado en la esquina superior derecha del panel. Ademas también es opcional elegir una de las variables que va a actuar como etiqueta, en la figura de abajo no se puede apreciar porque esta funcionalidad fue añadida después.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/VisualsSelector.png}
    \caption{Selector de variables para visualizacion}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Diagramas/diagrama secuencia visualizacion.png}
    \caption{Diagrama de secuencia para usar el microservicio de importacion}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/DataButton.png}
    \caption{Botón de manejo de datos}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/RemoteLoading.png}
    \caption{Menu para carga de datos remota}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/ManyDatasetsLoad.png}
    \caption{Menu de carga de multiples archivos}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/ImportManager1.png}
    \caption{Menú importación de datos parte 1}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/ImportManger2.png}
    \caption{Menú importación de datos parte 2}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/ImporterLoadedData.png}
    \caption{Menú importación de datos con un archivo para subir al servidor}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/Importerready.png}
    \caption{Menu de importación listo para generar un grafo}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Diagramas/secuencia.png}
    \caption{Diagrama de secuencia para usar el microservicio de importacion}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/ExportFormats.png}
    \caption{Distintos formatos para exportar un grafo cargado en el servidor}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/ModelsButton.png}
    \caption{Menu boton de modelos}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/OntologyModel.png}
    \caption{Distintos modelos de ontología soportados por Apache Jena}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/InferenceModels.png}
    \caption{Distintos modelos de inferencia soportados por Apache Jena}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes Aplicacion/GeneralInferenceModel.png}
    \caption{Gestor de reglas para modelo de inferencia genérico}
...
Este documento constituye la documentación técnica completa de un sistema dockerizado para la gestión, consulta y visualización de grafos de conocimiento. El sistema se compone de un \textit{frontend} construido con Vite, React y TypeScript; un \textit{backend en Java} que utiliza Apache Jena para el razonamiento sobre grafos y Spring Boot para la exposición de servicios REST; y un \textit{backend en Python} basado en Flask/Gunicorn\cite{flask}\cite{gunicorn}, responsable de la importación de datos y de las transformaciones para su visualización. Esta documentación detalla la arquitectura, los componentes principales, los flujos de datos y las interacciones entre módulos.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1 \linewidth]{Diagramas/Arquitectura.png}
    \caption{Arquitectura del Sistema}
\end{figure}
\begin{itemize}
    \item \textit{Frontend}: Aplicación web interactiva construida con el framework Vite, utilizando React y TypeScript. Sirve una interfaz de usuario estática mediante \texttt{http-server}.
    \item \textit{Backend Java}: Servicio principal que maneja toda la lógica de ontologías y consultas. Utiliza \textit{Apache Jena} para las operaciones sobre el grafo RDF y el razonamiento, y \textit{Spring Boot} para estructurar la aplicación y exponer una API REST.
    \item \textit{Backend Python}: Conjunto de dos microservicios que amplían la funcionalidad:
    \begin{itemize}
        \item \textit{Importer}: Gestiona la importación de datos desde formatos estructurados (CSV, JSON, Excel, XML) y su conversión a RDF.
        \item \textit{Visualizer}: Realiza transformaciones de datos, específicamente reducción dimensional (PCA, t-SNE, UMAP), para habilitar visualizaciones avanzadas.
    \end{itemize}
    Ambos servicios están implementados con \textit{Flask} y desplegados con \textit{Gunicorn}.
\end{itemize}
...
\end{lstlisting}
\begin{itemize}
    \item \textit{SparqlQueryExecutorFactory}: Analiza el tipo de consulta SPARQL y devuelve la instancia adecuada de un ejecutor concreto.
    \item \textit{SparqlQueryExecutor}: Interfaz que define el contrato común (\texttt{execute()}).
    \item \textit{Ejecutores Concretos}:
    \begin{itemize}
...
\begin{description}
    ...
    \item[InferenceManipulator:] Gestiona la creación de modelos de inferencia sobre datos RDF existentes. Permite usar \textit{reasoners} predefinidos (RDFS, OWL) o reglas personalizadas escritas en sintaxis Jena. Ofrece funcionalidades avanzadas como validación de consistencia, listado de triplas inferidas y explicación de derivaciones (\texttt{explainDerivation}).
\end{description}

\subsubsection{SparqlService}
Es el servicio central (anotado con \texttt{@Service}) que orquesta las operaciones principales:
\begin{itemize}
    \item \textit{Ejecución de consultas}: Expone el método \texttt{runSparqlQuery(String)} que sigue el flujo descrito anteriormente.
    \item \textit{Carga de datos}: Métodos para cargar o añadir datos RDF al dataset central desde archivos, URLs o streams, soportando múltiples formatos (RDF/XML, Turtle, JSON-LD, etc.).
    \item \textit{Exportación}: Permite exportar el grafo completo a diversos formatos de serialización RDF.
    \item \textit{Gestión del Dataset}: Mantiene el dataset Jena que contiene todos los grafos cargados.
\end{itemize}
...
\begin{description}
    ...
    \item[InfModelController:] Expone endpoints en \texttt{/infmodel/*} para operaciones de inferencia: crear modelos con \textit{reasoners} predefinidos o reglas genéricas, validar consistencia, listar triplas inferidas y explicar derivaciones.
\end{description}
...
1. \textit{Carga}: Los archivos se suben y almacenan en memoria como bytes.
2. \textit{Conversión}: Se detecta el tipo por extensión y se parsea con la librería adecuada (Pandas). Para CSV, se detecta automáticamente el delimitador.
3. \textit{Exportación RDF}: Cada fila del DataFrame se convierte en un sujeto con URI única (\texttt{http://example.org/filename\_index}). Cada columna no vacía se convierte en un predicado, y su valor en un objeto literal. Se genera un documento Turtle unificado.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Diagramas/estadosVisual.png}
    \caption{Diagrama de estados del servicio de visualización}
\end{figure}
\subsubsection{Propósito y Funcionalidad}
\begin{itemize}
...
\begin{itemize}
    \item \textit{Preprocesamiento}: Detección automática de columna de etiqueta. Las columnas se clasifican en:
    \begin{itemize}
    ...
    \end{itemize}
    \item \textit{Algoritmos}: Configuraciones por defecto para PCA, t-SNE (perplejidad: 30) y UMAP (vecinos: 15, distancia mínima: 0.1).
\end{itemize}

\subsubsection{Flujo de la API}
\begin{enumerate}
    \item \textbf{POST /upload}: El cliente envía un CSV y parámetros. El servidor preprocesa los datos, genera un \texttt{job\_id} y los almacena, pero \textit{no calcula embeddings aún}.
...
Los resultados se guardan en un diccionario global \texttt{RESULTS} indexado por \texttt{job\_id}, que contiene los datos preprocesados, parámetros, la columna de etiqueta y un caché de embeddings ya calculados. Actualmente como el sistema es para un solo usuario cada vez que se suben nuevos datos los anteriores se borran
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{Diagramas/Jerarquia.png}
     \caption{Jerarquía de componentes del frontend}
\end{figure}
...
\begin{itemize}
    ...
    \item \texttt{/builder}: Constructor de triplas RDF.
...
\begin{description}
    ...
    \item[triple-builder.tsx:] Constructor visual de triplas RDF para generar sentencias \texttt{INSERT DATA}. Maneja prefijos y tipado automático de literales.
\end{description}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{Diagramas/Entidades.png}
    \caption{Diagrama de entidades principales del frontend}
\end{figure}
\subsection{Comunicación entre Componentes}
\begin{itemize}
    \item \textit{Frontend $\leftrightarrow$ Backend Java (Puerto 8080)}: Para todas las operaciones SPARQL, gestión del grafo RDF, y creación de modelos de razonamiento. Para el resto de proveedores de servicios SPARQL se hace una consulta directamente y se formatean los resultados, el resto de funcionalidades estan desactivadas cuando el proveedor no es el por defecto.
    \item \textit{Frontend $\leftrightarrow$ Backend Python - Importer (Puerto 5001)}: Para subir archivos estructurados y obtener su representación RDF.
    \item \textit{Frontend $\leftrightarrow$ Backend Python - Visualizer (Puerto 5000)}: Para enviar datos y obtener \textit{embeddings} 2D/3D.
\end{itemize}
...
\subsection{Proveedores de servicios de SPARQL}

La aplicación integra un módulo flexible para la ejecución de consultas SPARQL sobre diferentes puntos de acceso remotos, permitiendo al usuario cambiar dinámicamente el proveedor de datos sin modificar el resto del sistema. Esta funcionalidad se organiza en dos piezas clave: un contexto de React que almacena la configuración del proveedor activo, y un ejecutor que despacha cada consulta al \textit{endpoint} correspondiente adaptando el formato de la petición y la interpretación de la respuesta.

El contexto \textit{SparqlContext} (archivo \texttt{SparqlContext.tsx}) define una lista de proveedores preconfigurados: \textbf{Know Graph}, el \textit{backend} interno que espera la consulta en el cuerpo de una petición POST y devuelve resultados en CSV; \textbf{Wikidata}, que utiliza el SDK \texttt{wikibase-sdk} para construir la URL codificada y recibe JSON estándar de SPARQL; y \textbf{DBpedia}, que emplea una petición GET con el parámetro \texttt{query} y requiere explícitamente el formato CSV. Cada proveedor se modela con un identificador, una etiqueta descriptiva, la URL del \textit{endpoint}, un indicador booleano que determina si la consulta viaja como carga útil (\texttt{queryOnPayload}) o como parámetro de consulta, una ruta adicional (\texttt{queryPath}) opcional para \textit{backends} internos, y el formato de respuesta esperado (\texttt{csv} o \texttt{json}). Además, se expone una función \texttt{setCustomProvider} que permite al usuario agregar en tiempo de ejecución cualquier punto SPARQL externo, especificando la URL, el modo de envío y el formato de salida, con lo que se cubre el caso de proveedores no contemplados inicialmente.

El módulo \texttt{sparqlExecutor.ts} se encarga de materializar las consultas. Para cada proveedor predefinido existe una función ejecutora especializada que construye la petición HTTP adecuada y parsea la respuesta. \textbf{Wikidata}, por ejemplo, delega en la instancia de \texttt{wikibase-sdk} la generación de la URL y posteriormente convierte el JSON SPARQL a una estructura homogénea mediante \texttt{parseSparqlJson}. \textbf{Know Graph} y \textbf{DBpedia} reciben CSV y lo transforman con \textit{PapaParse} a través de \texttt{parseCsv}. Cuando el proveedor es de tipo \texttt{custom} o no se encuentra en el registro, se emplea un ejecutor genérico que respeta los campos \texttt{queryOnPayload}, \texttt{queryPath} y \texttt{responseFormat} para decidir si envía GET o POST y cómo interpretar la respuesta. Todos los ejecutores retornan un objeto \texttt{SparqlResult} estandarizado, que incluye la lista de variables, un arreglo de filas con pares clave‑valor y la respuesta cruda, este sistema de proveedor no registrado esta en desarrollo y puede no funcionar en la mayoría de los casos.


\subsection{Flujo de Trabajo Típico}
\begin{enumerate}
    \item \textit{Importar Datos}: El usuario sube un archivo CSV mediante el \texttt{import-manager} (Python Importer) o carga un grafo RDF existente mediante el \texttt{data-button} (Java).
    \item \textit{Consultar/Explorar}: Construye una consulta en el \texttt{query-wizard} o \texttt{executor} y la ejecuta. Los resultados tabulares quedan disponibles.
    \item \textit{Visualizar}: Desde el \texttt{visualization-button}, elige:
    \begin{itemize}
        \item \textit{Gráfico del Grafo}: Selecciona variables para nodos/enlaces y visualiza en 2D/3D.
        \item \textit{Reducción Dimensional}: Selecciona columnas y algoritmo. Los datos se envían al Visualizer de Python y los resultados se muestran en las vistas 2D/3D.
    \end{itemize}
    \item \textit{Exportar/Guardar}: Usa el \texttt{data-button} para exportar el grafo de conocimiento o los resultados en diversos formatos.
\end{enumerate}
...
\begin{itemize}
    \item \textit{importer-service}: Contiene el microservicio Python de importación de datos (puerto 5001).
    \item \textit{visualizer-service}: Contiene el microservicio Python de reducción dimensional (puerto 5000).
    \item \textit{sparql-backend-service}: Contiene la aplicación Java Spring Boot con Apache Jena (puerto 8080).
    \item \textit{frontend-service}: Contiene la aplicación React/Vite servida mediante \texttt{http-server} (puerto 80).
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{Diagramas/EsquemaDeploy.png}
    \caption{Diagrama de despliegue de la aplicacion con docker compose}
\end{figure}
...
\begin{enumerate}
    \item \textit{Construcción de imágenes}:
    \begin{lstlisting}[language=bash, basicstyle=\small\ttfamily]
    docker-compose build
    \end{lstlisting}

    \item \textit{Inicio de servicios}:
    \begin{lstlisting}[language=bash, basicstyle=\small\ttfamily]
    docker-compose up -d
    \end{lstlisting}

    \item \textit{Monitoreo}:
    \begin{lstlisting}[language=bash, basicstyle=\small\ttfamily]
    docker-compose logs -f
    \end{lstlisting}

    \item \textit{Detención}:
    \begin{lstlisting}[language=bash, basicstyle=\small\ttfamily]
    docker-compose down
    \end{lstlisting}
\end{enumerate}
...
\begin{itemize}
    \item \textit{Aislamiento de Dependencias}: Cada servicio tiene sus propias dependencias (Java JDK, Python 3.9, Node.js) sin conflictos.

    \item \textit{Escalabilidad Independiente}: Los servicios pueden escalarse por separado según la carga (ej: múltiples instancias del visualizer).

    \item \textit{Portabilidad}: El sistema completo puede ejecutarse en cualquier host con Docker, sin necesidad de instalar manualmente Java, Python o Node.js.

    \item \textit{Versionado Consistente}: Las versiones exactas de cada dependencia se fijan en los Dockerfiles, garantizando reproducibilidad.

    \item \textit{Integración con CI/CD}: La contenerización facilita la implementación de pipelines de integración y despliegue continuo.
\end{itemize}

\subsection{Consideraciones de Configuración}
\begin{itemize}
    \item \textit{Persistencia de Datos}: Los contenedores son efímeros por defecto. Para producción, se deberían añadir volúmenes Docker para datos persistentes.

    \item \textit{Seguridad}: Los contenedores se ejecutan con usuarios no privilegiados cuando es posible, y solo los puertos necesarios están expuestos.

    \item \textit{Rendimiento}: La red bridge ofrece buen rendimiento para comunicación inter-servicios, con latencia mínima.

    \item \textit{Recursos}: Es posible limitar CPU y memoria por contenedor mediante configuraciones adicionales en el docker-compose.
\end{itemize}
...
\end{lstlisting}
\textit{Características clave:}
\begin{itemize}
\item \textit{Tamaño reducido}: Utiliza la variante \texttt{alpine} de Node.js para minimizar la imagen final.
\item \textit{Construcción separada}: Instala las dependencias de producción, luego instala globalmente TypeScript y Vite necesarios para la construcción (\texttt{build}), y finalmente compila la aplicación React.
\item \textit{Servidor estático}: Utiliza \texttt{http-server} para servir los archivos estáticos generados en la carpeta \texttt{dist} en el puerto 80.
\item \textit{Optimización}: La instalación de dependencias se realiza antes de copiar el código fuente para aprovechar la caché de capas de Docker.
\end{itemize}
...
\textit{Características clave:}
\begin{itemize}
\item \textit{Construcción multi-etapa}: La primera etapa (\texttt{build}) usa Maven para compilar el proyecto y generar el archivo JAR. La segunda etapa (\texttt{runtime}) utiliza solo el JDK para ejecutar la aplicación.
\item \textit{Optimización de dependencias}: El comando \texttt{mvn dependency:go-offline} descarga y cachea las dependencias, acelerando construcciones posteriores.
\item \textit{Imagen de ejecución ligera}: La etapa de runtime solo incluye el JAR y el JDK, sin las herramientas de construcción (Maven).
\item \textit{Java 21}: Utiliza la distribución Eclipse Temurin de Java 21, asegurando compatibilidad con las últimas características del lenguaje y rendimiento.
\end{itemize}
...
\textit{Características clave:}
\begin{itemize}
\item \textit{Imagen base ligera}: Se utiliza la variante \texttt{slim} de Python 3.13.5 y \texttt{alpine}.
\item \textit{Dependencias del sistema}: Se instalan compiladores y bibliotecas matemáticas (\texttt{libopenblas-dev}) necesarias para el rendimiento óptimo de NumPy, SciPy y scikit-learn utilizados en los algoritmos de reducción dimensional.
\item \textit{Gestión de dependencias Python}: Se copia el archivo \texttt{requirements.txt} y se instalan las dependencias con \texttt{pip}, utilizando la bandera \texttt{--no-cache-dir} para reducir el tamaño de la imagen.
...
\end{itemize}
\item \textit{Healthcheck integrado}: Cada contenedor incluye una verificación de salud a nivel de Docker que prueba la disponibilidad del endpoint raíz del servicio.
\item \textit{Servidor de producción}: Se utiliza \texttt{gunicorn} como servidor WSGI con configuración predefinida (1 worker, 4 threads) para manejar múltiples peticiones concurrentes de manera eficiente.
\end{itemize}
...
\begin{itemize}
\item \textit{Dependencias Aisladas}: Cada servicio tiene su propio entorno con versiones específicas de Node.js, Java o Python, evitando conflictos.
\item \textit{Construcciones Reproducibles}: Los \texttt{Dockerfile} garantizan que cada construcción produzca una imagen idéntica, independientemente del entorno del desarrollador.
\item \textit{Despliegue Eficiente}: Las imágenes resultantes están optimizadas en tamaño (especialmente la etapa final del backend Java y el uso de variantes \texttt{alpine}/\texttt{slim}) y listas para producción.
\end{itemize}
...
\begin{itemize}
\item \textit{Usuarios no root}: Los servicios Python se ejecutan como un usuario sin privilegios (\texttt{appuser}). El frontend, al usar \texttt{http-server} en Alpine, también se ejecuta de manera no privilegiada por defecto. El backend Java hereda la configuración de usuario de la imagen base de Eclipse Temurin.
\item \textit{Healthchecks}: Implementados directamente en los Dockerfiles de Python y referenciados en el compose, permiten a Docker y a orquestadores supervisar el estado del servicio.
\end{itemize}

\section{Uso de Nginx como proxy inverso}

Para facilitar la comunicación entre el frontend y los distintos microservicios backend, se ha implementado un servidor \texttt{Nginx} que actúa como proxy inverso. Esta decisión arquitectónica permite centralizar el enrutamiento de las peticiones HTTP, mejorar la seguridad y simplificar la configuración del cliente.

\subsection{Arquitectura de red y servicios}

La aplicación se despliega mediante \texttt{Docker Compose}, creando una red interna denominada \texttt{sparql-network} en la que conviven los siguientes contenedores:

\begin{itemize}
    \item \texttt{frontend-service}: sirve los archivos estáticos de la aplicación React.
    \item \texttt{sparql-backend-service}: backend en Java Spring Boot que gestiona consultas SPARQL y manipulación de datasets.
    \item \texttt{importer-service}: backend en Python Flask encargado de importar datos estructurados y convertirlos a RDF.
    \item \texttt{visualizer-service}: backend en Python Flask que calcula embeddings mediante técnicas de reducción de dimensionalidad (PCA, t‑SNE, UMAP).
\end{itemize}

El contenedor de Nginx forma parte del servicio \texttt{frontend} y expone el puerto 80 hacia el exterior, actuando como puerta de entrada única para todas las peticiones.

\subsection{Configuración del proxy inverso}

El archivo \texttt{nginx.conf} define las reglas de enrutamiento. A continuación se muestra un fragmento relevante:
...
Cada bloque \texttt{location} captura las rutas que comienzan con un prefijo específico (\texttt{/api/sparql/}, \texttt{/api/importer/}, \texttt{/api/visualizer/}) y redirige la petición al contenedor correspondiente utilizando su nombre de servicio interno y el puerto expuesto. Las directivas \texttt{proxy\_set\_header} garantizan que la información original del cliente (dirección IP, protocolo) se transmita a los backend, lo que resulta útil para registros y depuración.

Además del proxy, Nginx también sirve los archivos estáticos de la aplicación React. La directiva \texttt{try\_files \$uri \$uri/ /index.html;} asegura que cualquier ruta no coincidente con los prefijos de API sea manejada por el enrutamiento del lado del cliente (SPA).

\subsection{Ventajas de este enfoque}

\begin{itemize}
    \item \textbf{Punto de entrada único}: El frontend solo necesita conocer la dirección del servidor Nginx; no requiere conocer las direcciones individuales de cada backend. Esto simplifica la configuración en el entorno de desarrollo y producción.
    \item \textbf{Separación de dominios}: Todas las peticiones a los backend se realizan bajo el mismo dominio y puerto, evitando problemas de CORS (Cross-Origin Resource Sharing) que surgirían si el frontend intentara acceder directamente a servicios en puertos diferentes.
    \item \textbf{Seguridad}: Los backend no están expuestos directamente a Internet; solo son accesibles a través de la red interna de Docker. Nginx actúa como un firewall de aplicaciones, pudiendo añadir capas adicionales de autenticación, limitación de tasa o registro de accesos.
    \item \textbf{Flexibilidad}: En el futuro, se podrían añadir más servicios backend sin modificar el frontend, simplemente agregando nuevas reglas de proxy en Nginx.
\end{itemize}

\subsection{Integración con Docker Compose}

En el archivo \texttt{docker-compose.yaml}, el servicio \texttt{frontend} se construye con una imagen que incluye Nginx y los archivos estáticos. Las variables de entorno \texttt{VITE\_*} definen las rutas relativas que el frontend utilizará para construir las URLs de las API (por ejemplo, \texttt{/api/sparql/runQuery}). Gracias al proxy inverso, estas rutas son transparentemente redirigidas al backend correspondiente.
...
De esta manera, la aplicación en el navegador realiza peticiones a rutas como \texttt{/api/sparql/runQuery}, que Nginx interpreta y redirige internamente a \texttt{http://sparql-backend:8080/runQuery}. Este diseño mantiene la coherencia y facilita el despliegue en entornos donde el puerto 80 es el estándar para el tráfico web. Otra configuración de interes es el timeout y tamaño de las consultas posibles de hacer que fueron modificadas de sus valores por defecto pues algunas respuestas a consultas al dataset pueden pesar mas de los esperado o al tratar de visualizar puede que la respuesta tarde en resolverse.
