...
\begin{itemize}
    ...
    \item \textit{Wikidata Query Creator}: vinculado a \textit{Wikidata}, una base de conocimiento abierta mantenida por la Fundación Wikimedia, permite realizar consultas complejas sobre una enorme red de entidades y relaciones utilizando SPARQL, facilitando la extracción de información interconectada y su análisis semántico. 
...
Linked Data es un conjunto de buenas prácticas para la publicación y conexión de datos estructurados en la Web, con el objetivo de facilitar su reutilización e interoperabilidad. Este enfoque se basa en el uso de estándares abiertos que permiten que los datos sean comprensibles tanto para humanos como para máquinas, promoviendo una Web de datos interconectados en lugar de documentos aislados.

Los principios fundamentales de Linked Data incluyen el uso de identificadores URI para nombrar recursos, el acceso a dichos recursos mediante el protocolo HTTP, la representación de la información utilizando modelos de datos estándar como RDF, y la inclusión de enlaces hacia otros recursos relacionados. Estos principios permiten integrar información proveniente de diferentes fuentes, creando un ecosistema de datos enlazados que puede ser explorado y combinado de manera automática.

La adopción de Linked Data aporta beneficios como una mayor interoperabilidad entre sistemas, la mejora en la calidad y el contexto de los datos, y la posibilidad de descubrir nuevo conocimiento a partir de la conexión entre conjuntos de datos distribuidos. Por estas razones, Linked Data es un componente clave dentro de la Web Semántica y de las iniciativas modernas de gestión y publicación de datos.
...
\section{Knowledge Graph de Google}
...
Cada uno de estos componentes puede ser identificado mediante un \textit{Identificador de Recursos Uniforme (URI)}, lo que permite referenciar inequívocamente recursos distribuidos a lo largo de la Web. El objeto también puede ser un \textit{valor literal} (por ejemplo, una cadena de texto, un número o una fecha).
...
\begin{itemize}
    \item \textit{Consulta de repositorios RDF y triplestores}:  
    ...
    \item \textit{Integración semántica de datos}:  
    ...
    \item \textit{Exploración y análisis de grafos de conocimiento}:  
    ...
    \item \textit{Transformación y generación de conocimiento}:  
    ...
    \item \textit{Soporte para inferencia y razonamiento semántico}:  
...
\section{Vite, Typescript y React para construir un frontend profesional}
...
\begin{itemize}
    \item \textit{Frontend $\leftrightarrow$ Backend Java (Puerto 8080)}: Para todas las operaciones SPARQL, gestión del grafo RDF, y creación de modelos de razonamiento.
...
\textit{Características clave:}
\begin{itemize}
\item \textit{Imagen base ligera}: Se utiliza la variante \texttt{slim} de Python 3.13.5.
...
\item \textit{Servidor de producción}: Se utiliza \texttt{gunicorn} como servidor WSGI con configuración predefinida (2 workers, 4 threads) para manejar múltiples peticiones concurrentes de manera eficiente.
\end{itemize}
