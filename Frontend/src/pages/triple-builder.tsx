...
const PrefixSelectTrigger = styled(BaseSelectTrigger)`
  width: 80px;
  background-color: rgba(248, 250, 252, 0.9); /* light background */
  border: 1px solid rgba(226, 232, 240, 0.8); /* light border */
  color: rgba(17, 24, 39, 0.9); /* dark text */
  font-size: 0.875rem;
  transition: background-color 0.2s, border-color 0.2s;

  &:hover {
    background-color: rgba(229, 231, 235, 0.7); /* light hover effect */
  }

  &:focus {
    outline: none;
    ring: 2px solid rgba(16, 185, 129, 0.5); /* Green focus ring */
  }
`
...
const WideInput = styled(BaseInput)`
  ...
  &:focus {
    outline: none;
    border-color: rgba(16, 185, 129, 0.5); /* Green focus border */
    ring: 2px solid rgba(16, 185, 129, 0.5); /* Green focus ring */
  }
`
...
export function TripleBuilder({ setQuery, setMessage }: TripleBuilderProps) {
  ...
  return (<div className="max-w-6xl mx-auto space-y-6 p-6 bg-white rounded-xl shadow-2xl shadow-black/30">
  <PrefixManager
    prefixes={prefixes}
  ...
  <div className="grid gap-6 lg:grid-cols-2">
    <Card className="bg-white border border-zinc-200 shadow-lg shadow-zinc-500/20 backdrop-blur-sm">
      <CardHeader className="border-b border-zinc-200">
        <CardTitle className="text-zinc-900 text-2xl font-semibold">Build New Triple</CardTitle>
      ...
      <CardContent className="space-y-6 pt-6">
        {[
          ...
          <div key={field} className="space-y-2">
            <Label htmlFor={field} className="text-zinc-700 font-medium ">
              {label}
            </Label>
            <div className="flex gap-2">
              ...
              >
                <PrefixSelectTrigger className="bg-red-500">
                  <SelectValue placeholder="Prefix..." />
                </PrefixSelectTrigger>
                <SelectContent className="bg-white border border-zinc-300 text-zinc-900 ">
                  <SelectItem value="" className="text-zinc-700 hover:bg-zinc-300">
                    None
                  </SelectItem>
                  {prefixes.map((prefix) => (
                    <SelectItem
                      key={prefix.id}
                      value={prefix.prefix}
                      className="text-zinc-700 font-mono hover:bg-zinc-300"
                    >
              ...
              >
                <BaseSelectTrigger className="w-[100px] bg-white border border-zinc-300 text-zinc-900 focus:ring-2 focus:ring-emerald-500/50 hover:bg-zinc-300">
                  <SelectValue placeholder="Select..." />
                </BaseSelectTrigger>
                <SelectContent className="bg-white border border-zinc-300 ">
                  {options.map((option) => (
                    <SelectItem key={option} value={option} className=" hover:bg-zinc-300 text-zinc-700">
                      {option}
                    </SelectItem>
                  ))}
                </SelectContent>
        ...
        <div className="flex gap-3">
          ...
          <Button
            onClick={resetForm}
            variant="outline"
            className="border-zinc-500 text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900 focus:ring-2 focus:ring-emerald-500/50"
          >
    ...
    <Card className="bg-white border border-zinc-200 shadow-lg shadow-zinc-500/20 backdrop-blur-sm">
      <CardHeader className="flex flex-row items-center justify-between border-b border-zinc-200">
        <div>
          <CardTitle className="text-zinc-900 text-2xl font-semibold">Built Triples</CardTitle>
          <CardDescription className="text-zinc-600">
        ...
        <div className="space-y-4">
          {triples.length === 0 ? (
            ...
            triples.map((triple) => (
              <div
                key={triple.id}
                className="p-4 rounded-lg bg-white border border-zinc-300 hover:border-emerald-500/50 transition-colors"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-zinc-900 font-mono text-sm">{triple.subject}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="bg-blue-500/10 text-blue-400 border-blue-500/30">
                        Predicate
                      </Badge>
                      <span className="text-zinc-900 font-mono text-sm">{triple.predicate}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="bg-purple-500/10 text-purple-400 border-purple-500/30">
                        Object
                      </Badge>
                      <span className="text-zinc-900 font-mono text-sm">{triple.object}</span>
                    </div>
                  </div>
                  <Button
                    variant="ghost"
  ...
  <Card className="bg-white border border-zinc-200 shadow-lg shadow-zinc-500/20 backdrop-blur-sm">
    <CardHeader className="border-b border-zinc-200">
      <CardTitle className="text-zinc-900 text-2xl font-semibold">Graph Selection & Query Generation</CardTitle>
      <CardDescription className="text-zinc-600">
    ...
    <CardContent className="space-y-6 pt-6">
      <div className="space-y-2">
        <Label htmlFor="graph" className="text-zinc-700 font-medium">
          Graph URI
        </Label>
        <div className="flex gap-2">
          <WideInput
            ...
            className=" flex-1 bg-white border border-zinc-300 text-zinc-900 placeholder:text-zinc-500/60 focus:ring-2 focus:ring-emerald-500/50"
          />
          <Select
          ...
          >
            <GraphSelector className="w-[300px] bg-white border border-zinc-300 text-zinc-900 focus:ring-2 focus:ring-emerald-500/50">
              <SelectValue placeholder="Select graph..." />
            </GraphSelector>
            <SelectContent className="bg-white border border-zinc-300">
              {mockGraphs.map((g) => (
                <SelectItem key={g} value={g} className="text-zinc-700 hover:bg-zinc-300">
                  {g}
                </SelectItem>
              ))}
            </SelectContent>
      ...
      {generatedQuery && (
        <div className="space-y-2">
          <Label className="text-sm text-zinc-700 font-medium">Generated Query</Label>
          <Textarea
            readOnly
            value={generatedQuery}
            rows={8}
            className="bg-white border border-zinc-300 text-zinc-900 font-mono text-sm resize-none focus:ring-2 focus:ring-emerald-500/50"
          />
        </div>
      )}
    </CardContent>
